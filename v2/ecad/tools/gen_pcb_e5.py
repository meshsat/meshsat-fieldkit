...
13.4 mm blind-mate gap. Top: the 2 x 6 signal targets under A22's J_DOCK (-124, -70); their nets are copied from the A22 board by position
(A22's connector sits on its underside, so its pin order is mirrored on the block), four CELL+ targets, four return targets and the
pre-charge target. Twelve plated wire lands (2 x 6 at 2.54 mm, 1.0 mm holes) at (-124, -76.5) carry the same nets straight down each
...
the strip's P_CP / P_CN. No schematic: nets are named here. 2 oz copper, 43 x 26 mm, X -158..-115, Y -85..-59. Each contact is its own net (name plus the A22 pin number): the strip's J_BLK is where the repeated pins meet.
...
A22_PCB = os.path.normpath(os.path.join(PRJDIR, "..", "pcb-a-power", "pcb-a-power.kicad_pcb"))
...
X0, X1, Y0, Y1, R = -106.5, -63.5, -85.0, -59.0, 1.5   # E6 (32.56): under A22's dock pin field east of the west rod
HOLES = [(-104.0, -63.0), (-66.0, -63.0), (-104.0, -83.0), (-66.0, -83.0)]      # = BLOCK_HOLES of gen_pcb_e.py (the strip's standoffs), in the four corners so nothing crowds the contact field
board = pcbnew.BOARD()
...
# --- A22's J_DOCK pin under every target, by position (the A22 connector is flipped to its underside; whatever KiCad's flip does, the match is geometric)
if not os.path.exists(A22_PCB): raise SystemExit("A22 board not found, the block copies its pin order from it: " + A22_PCB)
a22b = pcbnew.LoadBoard(A22_PCB); jd = a22b.FindFootprintByReference("J_DOCK")
...
for pad in tgt.Pads():
    ...
    if key not in a22: raise SystemExit("no A22 pin above block target pad %s" % pad.GetNumber())
    num, n = a22[key]; n = n.lstrip("/")
    if not n or n.startswith("unconnected-"): n = "SPARE"
    # one net per contact: the block passes each pin through to its own wire, and the strip's J_BLK is where the four
    # SHORE_12V and the four GND pins meet. Bussing them here instead would need a bus layer the 43 x 26 board has not got.
    PIN[int(pad.GetNumber())] = (num, n); pad.SetNet(net("%s_P%d" % (n, num)))
land = place("meshsat", "WireLands_2x6", "L_SIG", -76.0, -76.5, "plated wire lands to the strip J_BLK (net names on the underside silk)")
for pad in land.Pads():
    num, n = PIN[int(pad.GetNumber())]; pad.SetNet(net("%s_P%d" % (n, num)))
for k in range(4):
    x = -99.0 + 4.0 * k
...
t = place("meshsat", "Mill-Max_0858_target", "T_PRE", -103.0, -70.0, "pre-charge target (longer pin, mates first)"); [p.SetNet(net("CELL+")) for p in t.Pads()]
...
rounded_rect(-105.6, -75.8, -84.4, -64.4, 2.0, pcbnew.F_SilkS, 0.12)
w = place("meshsat", "WireHole_2mm", "WH_CP", -92.0, -81.0, "12 AWG to the strip P_CP"); [p.SetNet(net("CELL+")) for p in w.Pads()]
w = place("meshsat", "WireHole_2mm", "WH_CN", -93.0, -62.5, "12 AWG to the strip P_CN"); [p.SetNet(net("CELL_N")) for p in w.Pads()]
...
SHORT = {"VIN_RAW": "VIN", "GND": "GND", "SHORE_INHIBIT": "INH", "USB_E6_P": "D+", "USB_E6_N": "D-", "DOCK_SPARE": "SP", "SPARE": "SP"}
def label(k, x, y):
...
for k in range(1, 7):
    ...
    track(x, yn, x, -66.5, 0.4, pcbnew.F_Cu, n); via(x, -66.5, n); track(x, -66.5, x, yln, 0.4, pcbnew.B_Cu, n)
    n = tp[k + 6].GetNetname(); x, ys = dxy(tp[k + 6]); _, yls = dxy(lp[k + 6]); xj = x + 1.27
    pts = [(x, ys), (x, -73.0), (xj, -74.0), (xj, -76.6), (x, yls)]
    for (x1, y1), (x2, y2) in zip(pts, pts[1:]): track(x1, y1, x2, y2, 0.3, pcbnew.F_Cu, n)
    label(k, dxy(tp[k])[0], -71.5); label(k + 6, dxy(tp[k + 6])[0], -81.0)
...
print("block target / land k -> A22 J_DOCK pin, net (wire land k to the strip J_BLK pin of that number):")
