#!/usr/bin/env python3
"""Board E's own footprints, written into meshsat.pretty (MESHSAT-1357 round 4, 26 September 2026). Today one: the
Sensirion SGP41-D-R4 of item S-10.
...
Board E declares this generator in boards/e.json `footprint_generator`, so full.sh rebuilds the land from the tree
(test_footprint_library: an artefact that exists only on a rented machine exists nowhere).
Usage: gen_footprints_e.py <meshsat.pretty dir>
"""
import os, sys

NAME = "Sensirion_DFN-6-1EP_2.44x2.44mm_P0.8mm_EP1.25x1.7mm"
OUT = sys.argv[1] if len(sys.argv) > 1 else os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "meshsat.pretty")
