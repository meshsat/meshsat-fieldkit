...
lines = ["(rules PCB %s" % name, "  (snap_angle fortyfive_degree)",
         "  (autoroute_settings (fanout %s) (autoroute %s) (postroute %s) (vias on) (via_costs %s) (plane_via_costs %s) (start_ripup_costs %s) (start_pass_no 1)" % (
             arg("--fanout", "off"), arg("--autoroute", "on"), arg("--postroute", "on"), arg("--via-costs", "50"), arg("--plane-via-costs", "5"), arg("--ripup", "100"))]
...
lines += ["  )", ")"]
open(out, "w").write("\n".join(lines) + "\n")
print("fr_rules: %s layers %s via_costs %s plane_via_costs %s ripup %s preferred %s inactive %s -> %s" % (name, layers, arg("--via-costs", "50"), arg("--plane-via-costs", "5"), arg("--ripup", "100"), pref or "none", sorted(inactive) or "none", out))
