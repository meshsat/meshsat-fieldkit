...
EXCLUDE = {"pcb-a-power": {"J_DOCK", "J_PRE1", "RT1", "L2", "L3", "L4", "L5", "L6"} | {"J_CP%d" % k for k in range(1, 5)} | {"J_CN%d" % k for k in range(1, 5)} | {"J_BM%d" % k for k in range(1, 8)},
           "pcb-d-aprs": {"L1"},
           "pcb-c-display": {"SW_MAIN", "SW_PI", "SW_TEST", "SW_LIGHT", "SW_SOS", "SW_EMCON", "SW_ZERO", "EPD1"},
...
BOARDS = [  # (deliverable folder, file stem, project dir, title, phase, hand-fitted list)
    ...
    ("meshsat-pcb-b-revA-B12", "pcb-b-compute", "pcb-b-compute", "PCB-B COMPUTE", "B12", "Pi 5 alone on four M2.5 x 22 standoffs (HDMI edge west, header edge east, SD card south), three JST-VH rail leads from PCB-A into J_5V_M1, J_5V_M2 and J_5V_PI (the Pi lead ends in a USB-C plug), T-Call, T-Beam 1W, RTL-SDR / LimeSDR Mini, ZigBee dongle, RockBLOCK 9603 / 9704 with the GC bracket, Wio-SX1262, the 2x10 panel ribbon on J_PANEL and the 2x7 ribbon to PCB-A on J_AB1"),
    ("meshsat-pcb-c-revA-C4", "pcb-c-display", "pcb-c-display", "PCB-C CONTROL PANEL", "C4", "Touch Display 2 through the aperture on 0.05 mm transfer tape (3M 467MP / 9495LE), connector end to port; SW_MAIN C&K ATP19-SL1-603-B0SA-03G (19 mm, green ring), SW_PI C&K ATP16-SL1-403-M0SA-04G (16 mm, orange ring as the amber), SW_TEST C&K ATP16-SL1-203-M0SA-04G (16 mm, white ring), IP67 anti-vandal with flying leads to the four pads; SW_LIGHT NKK M2044SD3A01 DPDT ON-ON-ON toggle with the AT401A boot; SW_SOS, SW_EMCON and SW_ZERO APEM 5636ADKB-2V locking toggles (both positions locked, 1/4-40 bushing 9 mm, 6.5 mm cut-out, flying leads; SOS and ZEROIZE are maintained and the bridge times them, ruling 32.13); WeAct 3.7 e-paper module under the panel: glass into the recessed window, both PCB lands taped (3M 9495LE), optional 1.0 mm spacer ring for a flush face, 2x4 lead to J_EPD; 16 x M3 frame screws with the GND ring pads; XH leads from J_X1202SW and J_PIJ2"),
