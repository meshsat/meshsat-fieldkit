...
cd "$1/pcb-p-pack"; N=pcb-p-pack; LOG="$2"
rm -f out/p1-clean.txt out/par-score.txt out/contracts.log   # a stale clean flag must never finish a board (register class 6)
...
STUB_LAYERS=F.Cu,B.Cu STUB_GRID=0.1 nice -n 10 python3 ../tools/stub_router.py $N.kicad_pcb out/$N-drc.json > out/$N-stub.log 2>&1 || { echo "stub router CRASHED, exit $? (out/$N-stub.log)"; echo open > out/p1-clean.txt; echo FINISH-P1-DONE; exit 1; }; grep -E 'closed|FAILED|stub_router|Error' out/$N-stub.log | head -12
kicad-cli pcb drc --severity-all --format json -o out/$N-drc.json $N.kicad_pcb >/dev/null 2>&1
...
kicad-cli pcb drc --severity-all --format json -o out/$N-drc.json $N.kicad_pcb >/dev/null 2>&1
python3 ../tools/hardset.py out/$N-drc.json post --flag out/p1-clean.txt --label 'routed-board gate' | sed 's/^hardset:/routed-board DRC:/'
...
if ! python3 ../tools/check_pcb_p.py $N.kicad_pcb 2>/dev/null | grep -q 'RESULT: ALL PASS'; then echo 'P1 GATE FAIL on the routed board'; echo open > out/p1-clean.txt; fi
...
python3 ../tools/dc_drop.py $N.kicad_pcb --json out/$N-dc_drop.json > out/$N-dc_drop.log 2>&1; DC=$?; grep -E 'dc_drop' out/$N-dc_drop.log | tail -14; [ "$DC" -eq 0 ] || { echo 'P1 DC DROP MISSED or unresolved (out/$N-dc_drop.log)'; echo open > out/p1-clean.txt; }
python3 ../tools/stackup_write.py $N.kicad_pcb 2>&1 | tail -1   # the routed board carries the stack for the read-back (8 Sep 2026)
python3 ../tools/impedance_check.py $N.kicad_pcb --json out/$N-impedance.json > out/$N-impedance.log 2>&1; IM=$?; grep -E 'impedance' out/$N-impedance.log | tail -14; [ "$IM" -eq 0 ] || { echo 'P1 IMPEDANCE MISSED (out/$N-impedance.log; openEMS on the missed pairs)'; echo open > out/p1-clean.txt; }
# 8 Sep 2026 (MESHSAT-862): the cross-board contracts are part of every finish (they were called by no chain before)
python3 ../tools/check_contracts.py .. > out/contracts.log 2>&1; grep -E 'FAIL|MISSING' out/contracts.log | head -12; if grep -q 'ALL CONTRACTS PASS' out/contracts.log; then echo 'contracts: ALL PASS'; else echo 'contracts: FAIL (out/contracts.log)'; echo open > out/p1-clean.txt; fi
CLEAN=$(cat out/p1-clean.txt); if [ "$CLEAN" != clean ]; then echo 'P1 NOT CLEAN, not finishing'; echo FINISH-P1-DONE; exit 1; fi
cd ..; ./tools/finish_board.sh pcb-p-pack pcb-p-pack - meshsat-pcb-p-revA-P1 2>&1 | tail -16
[ "${PIPESTATUS[0]}" -eq 0 ] || { echo "P1: finish_board REFUSED the deliverable (verify_deliverable or an export step failed)"; echo FINISH-P1-DONE; exit 1; }   # 8 Sep 2026: a pipeline's status is tail's, so a refused deliverable used to be reported finished
echo FINISH-P1-DONE
