...
cd "$1/pcb-d-aprs-d9"; N=pcb-d-aprs; LOG="$2"
rm -f out/d9-clean.txt out/par-score.txt out/contracts.log   # a stale clean flag must never finish a board (register class 6)
...
STUB_LAYERS=F.Cu,In2.Cu,B.Cu STUB_GRID=0.1 nice -n 10 python3 ../tools/stub_router.py $N.kicad_pcb out/$N-drc.json > out/$N-stub.log 2>&1 || { echo "stub router CRASHED, exit $? (out/$N-stub.log)"; echo open > out/d9-clean.txt; echo FINISH-D9-DONE; exit 1; }; grep -E 'closed|FAILED|stub_router|Error' out/$N-stub.log | head -12
../tools/drc.sh $N.kicad_pcb out/$N-drc.json
...
../tools/pair_match.sh "$PWD" $N check_pcb_d.py > out/pair-match.log 2>&1; PM=$?; grep -E "pair_match|WARN|PASS|meander" out/pair-match.log | cut -c1-140
if [ "$PM" -ne 0 ]; then
  mkdir -p out/audit; for pr in USB1 USB2 USB3 USB_D8; do python3 ../tools/pair_audit.py $N.kicad_pcb $pr out/audit/$pr.png 2>&1 | grep pair_audit; done
  echo 'PAIRS NOT MATCHED, not finishing (audit images in out/audit)'; echo open > out/d9-clean.txt; echo FINISH-D9-DONE; exit 1
fi
...
../tools/drc.sh $N.kicad_pcb out/$N-drc.json
python3 ../tools/hardset.py out/$N-drc.json post --flag out/d9-clean.txt --label 'routed-board gate' | sed 's/^hardset:/routed-board DRC:/'
...
if ! python3 ../tools/check_pcb_d.py $N.kicad_pcb 2>/dev/null | grep -q 'RESULT: ALL PASS'; then echo 'D9 GATE FAIL on the routed board'; echo open > out/d9-clean.txt; fi
...
python3 ../tools/dc_drop.py $N.kicad_pcb --json out/$N-dc_drop.json > out/$N-dc_drop.log 2>&1; DC=$?; grep -E 'dc_drop' out/$N-dc_drop.log | tail -14; [ "$DC" -eq 0 ] || { echo 'D9 DC DROP MISSED or unresolved (out/$N-dc_drop.log)'; echo open > out/d9-clean.txt; }
python3 ../tools/stackup_write.py $N.kicad_pcb 2>&1 | tail -1   # the routed board carries the stack for the read-back (8 Sep 2026)
python3 ../tools/impedance_check.py $N.kicad_pcb --json out/$N-impedance.json > out/$N-impedance.log 2>&1; IM=$?; grep -E 'impedance' out/$N-impedance.log | tail -14; [ "$IM" -eq 0 ] || { echo 'D9 IMPEDANCE MISSED (out/$N-impedance.log; openEMS on the missed pairs)'; echo open > out/d9-clean.txt; }
# 8 Sep 2026 (MESHSAT-862): the cross-board contracts are part of every finish (they were called by no chain before)
python3 ../tools/check_contracts.py .. > out/contracts.log 2>&1; grep -E 'FAIL|MISSING' out/contracts.log | head -12; if grep -q 'ALL CONTRACTS PASS' out/contracts.log; then echo 'contracts: ALL PASS'; else echo 'contracts: FAIL (out/contracts.log)'; echo open > out/d9-clean.txt; fi
CLEAN=$(cat out/d9-clean.txt); if [ "$CLEAN" != clean ]; then echo 'D9 NOT CLEAN, not finishing'; echo FINISH-D9-DONE; exit 1; fi
cd ..; ./tools/finish_board.sh pcb-d-aprs-d9 pcb-d-aprs - meshsat-pcb-d-revA-D10 2>&1 | tail -16
[ "${PIPESTATUS[0]}" -eq 0 ] || { echo "D9: finish_board REFUSED the deliverable (verify_deliverable or an export step failed)"; echo FINISH-D9-DONE; exit 1; }   # 8 Sep 2026: a pipeline's status is tail's, so a refused deliverable used to be reported finished
echo FINISH-D9-DONE
