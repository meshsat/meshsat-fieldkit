#!/usr/bin/env bash
# B19 (the I/O high-availability topology, ARCH-PCB-B-IOHA): wait for route_parallel.sh (PARALLEL-DONE in the log), dangling clean-up FIRST, then the stub router on what is really open (on the four routing layers),
...
cd "$1/pcb-b-compute-b19"; N=pcb-b-compute; LOG="$2"
rm -f out/b19-clean.txt out/par-score.txt out/contracts.log   # a stale clean flag must never finish a board (register class 6)
...
../tools/drc.sh $N.kicad_pcb out/$N-drc.json
STUB_LAYERS=F.Cu,In2.Cu,In3.Cu,B.Cu STUB_GRID=0.1 nice -n 10 python3 ../tools/stub_router.py $N.kicad_pcb out/$N-drc.json > out/$N-stub.log 2>&1 || { echo "stub router CRASHED, exit $? (out/$N-stub.log)"; echo open > out/b19-clean.txt; echo FINISH-B19-DONE; exit 1; }; grep -E 'closed|FAILED|stub_router|Error' out/$N-stub.log | head -12
...
if [ "$PM" -ne 0 ]; then
  ...
  echo 'B19 PAIRS NOT MATCHED, not finishing (audit images in out/audit)'; echo open > out/b19-clean.txt; echo FINISH-B19-DONE; exit 1
fi
../tools/drc.sh $N.kicad_pcb out/$N-drc.json
python3 ../tools/hardset.py out/$N-drc.json post --flag out/b19-clean.txt --label 'routed-board gate' | sed 's/^hardset:/routed-board DRC:/'
...
python3 ../tools/pruned_gate.py $N.kicad_pcb out/$N-pruned.txt 2>&1 | grep -E 'pruned_gate' | tail -6; python3 ../tools/pruned_gate.py $N.kicad_pcb out/$N-pruned.txt >/dev/null 2>&1 || { echo 'B19 PRUNED PAD NOT REACHED by the router'; echo open > out/b19-clean.txt; }
python3 ../tools/check_pcb_b.py $N.kicad_pcb > out/gate-$N.log 2>&1; GATE=$?; grep -E 'FAIL|RESULT|WARN|pieces|stitch|^verdict:' out/gate-$N.log | tail -14; [ "$GATE" -eq 0 ] || { echo "B19 GATE $(python3 ../tools/verdict.py read out/check_pcb_b.verdict.json 2>&1 | tail -1) on the routed board"; echo open > out/b19-clean.txt; };   # 11 Sep 2026 (MESHSAT-862, stage 0a): one run, and the exit code is the verdict. This gate used to run twice, the second time only to grep a string out of its stdout.
...
python3 ../tools/dc_drop.py $N.kicad_pcb --json out/$N-dc_drop.json > out/$N-dc_drop.log 2>&1; DC=$?; grep -E 'dc_drop' out/$N-dc_drop.log | tail -14; [ "$DC" -eq 0 ] || { echo 'B19 DC DROP MISSED or unresolved (out/$N-dc_drop.log)'; echo open > out/b19-clean.txt; }
python3 ../tools/stackup_write.py $N.kicad_pcb 2>&1 | tail -1   # the routed board carries the stack for the read-back (8 Sep 2026)
python3 ../tools/impedance_check.py $N.kicad_pcb --json out/$N-impedance.json > out/$N-impedance.log 2>&1; IM=$?; grep -E 'impedance' out/$N-impedance.log | tail -14; [ "$IM" -eq 0 ] || { echo 'B19 IMPEDANCE MISSED (out/$N-impedance.log; openEMS on the missed pairs)'; echo open > out/b19-clean.txt; }
# 8 Sep 2026 (MESHSAT-862): the cross-board contracts are part of every finish (they were called by no chain before)
python3 ../tools/check_contracts.py .. > out/contracts.log 2>&1; grep -E 'FAIL|MISSING' out/contracts.log | head -12; if grep -q 'ALL CONTRACTS PASS' out/contracts.log; then echo 'contracts: ALL PASS'; else echo 'contracts: FAIL (out/contracts.log)'; echo open > out/b19-clean.txt; fi
CLEAN=$(cat out/b19-clean.txt); if [ "$CLEAN" != clean ]; then echo 'B19 NOT CLEAN, not finishing'; echo FINISH-B19-DONE; exit 1; fi
cd ..; ./tools/finish_board.sh pcb-b-compute-b19 pcb-b-compute post_fix_b13.py meshsat-pcb-b-revA-B19 2>&1 | tail -16
[ "${PIPESTATUS[0]}" -eq 0 ] || { echo "B19: finish_board REFUSED the deliverable (verify_deliverable or an export step failed)"; echo FINISH-B19-DONE; exit 1; }   # 8 Sep 2026: a pipeline's status is tail's, so a refused deliverable used to be reported finished
echo FINISH-B19-DONE
