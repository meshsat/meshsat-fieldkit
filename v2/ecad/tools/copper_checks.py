...
def run(b, check, nets=None):
    ...
    for z in pz:
        ...
        for i in range(fp.OutlineCount()):
            o = fp.Outline(i)
            if not any(o.PointInside(a) for a in anchors): loose += 1
    ...
    for net, zs in bynet.items():
        ...
        for z in zs:
            ...
            linked = any(fz.Contains(q) and any(o.GetFilledPolysList(o.GetFirstLayer()).Contains(q) for o in zs if o.GetFirstLayer() != L) for q in links)
