...
python3 ../tools/check_pcb_b.py $N.kicad_pcb 2>&1 | grep -E 'FAIL|RESULT|WARN|pieces|stitch' | tail -14; if ! python3 ../tools/check_pcb_b.py $N.kicad_pcb 2>/dev/null | grep -q 'RESULT: ALL PASS'; then echo 'B16 GATE FAIL on the routed board'; echo open > out/b16-clean.txt; fi
