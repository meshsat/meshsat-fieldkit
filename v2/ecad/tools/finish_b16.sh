...
rm -f out/b16-clean.txt out/par-score.txt out/contracts.log   # a stale clean flag must never finish a board (register class 6)
...
python3 ../tools/check_pcb_b.py $N.kicad_pcb > out/gate-$N.log 2>&1; GATE=$?; grep -E 'FAIL|RESULT|WARN|pieces|stitch|^verdict:' out/gate-$N.log | tail -14; [ "$GATE" -eq 0 ] || { echo "B16 GATE $(python3 ../tools/verdict.py read out/check_pcb_b.verdict.json 2>&1 | tail -1) on the routed board"; echo open > out/b16-clean.txt; };   # 11 Sep 2026 (MESHSAT-862, stage 0a): one run, and the exit code is the verdict. This gate used to run twice, the second time only to grep a string out of its stdout.
...
python3 ../tools/dc_drop.py $N.kicad_pcb --json out/$N-dc_drop.json > out/$N-dc_drop.log 2>&1; DC=$?; grep -E 'dc_drop' out/$N-dc_drop.log | tail -14; [ "$DC" -eq 0 ] || { echo 'B16 DC DROP MISSED or unresolved (out/$N-dc_drop.log)'; echo open > out/b16-clean.txt; }
python3 ../tools/stackup_write.py $N.kicad_pcb 2>&1 | tail -1   # the routed board carries the stack for the read-back (8 Sep 2026)
python3 ../tools/impedance_check.py $N.kicad_pcb --json out/$N-impedance.json > out/$N-impedance.log 2>&1; IM=$?; grep -E 'impedance' out/$N-impedance.log | tail -14; [ "$IM" -eq 0 ] || { echo 'B16 IMPEDANCE MISSED (out/$N-impedance.log; openEMS on the missed pairs)'; echo open > out/b16-clean.txt; }
# 8 Sep 2026 (MESHSAT-862): the cross-board contracts are part of every finish (they were called by no chain before)
python3 ../tools/check_contracts.py .. > out/contracts.log 2>&1; grep -E 'FAIL|MISSING' out/contracts.log | head -12; if grep -q 'ALL CONTRACTS PASS' out/contracts.log; then echo 'contracts: ALL PASS'; else echo 'contracts: FAIL (out/contracts.log)'; echo open > out/b16-clean.txt; fi
...
[ "${PIPESTATUS[0]}" -eq 0 ] || { echo "B16: finish_board REFUSED the deliverable (verify_deliverable or an export step failed)"; echo FINISH-B16-DONE; exit 1; }   # 8 Sep 2026: a pipeline's status is tail's, so a refused deliverable used to be reported finished
