...
python3 ../tools/check_pcb_b.py $N.kicad_pcb > out/gate-$N.log 2>&1; GATE=$?; grep -E 'FAIL|RESULT|WARN|pieces|stitch|^verdict:' out/gate-$N.log | tail -14; [ "$GATE" -eq 0 ] || { echo "B17 GATE $(python3 ../tools/verdict.py read out/check_pcb_b.verdict.json 2>&1 | tail -1) on the routed board"; echo open > out/b17-clean.txt; };   # 11 Sep 2026 (MESHSAT-862, stage 0a): one run, and the exit code is the verdict. This gate used to run twice, the second time only to grep a string out of its stdout.
...
cd ..; ./tools/finish_board.sh pcb-b-compute-b17 pcb-b-compute post_fix_b13.py meshsat-pcb-b-revA-B18 2>&1 | tail -16
