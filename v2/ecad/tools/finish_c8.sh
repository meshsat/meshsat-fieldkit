...
[ "$GATE" -eq 0 ] || { echo "C8 GATE $(python3 ../tools/verdict.py read out/check_pcb_c.verdict.json 2>&1 | tail -1) on the routed board"; echo open > out/c8-clean.txt; }
...
cd ..; ./tools/finish_board.sh pcb-c-display-c8 pcb-c-display - meshsat-pcb-c-revA-C10 2>&1 | tail -16
