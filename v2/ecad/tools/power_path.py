...
def segments(intent, nets):
    ...
    for netname, nodelist in nets.items():
        for ref, _pin in nodelist: part_nets.setdefault(ref, set()).add(netname)
    out = []
    for rname, r in sorted(rails.items()):
        ...
        for load, amps in ends:
            ...
            for other, nodelist in sorted(nets.items()):
                ...
                nd = nodes.get(other)
                if nd is None: continue                      # undeclared entirely: derate's question, not this one
                ...
                if len(part_nets.get(load, ())) > 3: continue  # not a pass-through part: see part_nets above
                out.append(dict(node=other, rail=rn, through=load, amps=float(amps or 0),
...
def main(argv):
    ...
    for s in rows:
        print("power_path: %-14s carries %s's %.2f A through %s (pins %s on the rail, %s on it), declared a NODE at %.1f V"
              % (s["node"], s["rail"], s["amps"], s["through"], ",".join(s["pins_on_rail"]), ",".join(s["pins_on_node"]), s["v_max"]))
        if s["basis"]: print("              its basis: %s" % s["basis"])
    print("power_path: %d declared node(s) carry a declared rail's current through one of its load parts, "
          "of %d rail(s) and %d node(s)" % (len(rows), len(intent.get("rails") or {}), len(intent.get("nodes") or {})))
    return _w("power_path", _v.PASS, counts=dict(segments=len(rows), rails=len(intent.get("rails") or {}),
                                                 nodes=len(intent.get("nodes") or {})),
              evidence=["%s carries %s's %.2f A through %s" % (s["node"], s["rail"], s["amps"], s["through"]) for s in rows],
