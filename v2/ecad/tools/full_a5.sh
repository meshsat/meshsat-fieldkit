#!/usr/bin/env bash
# PCB-A A5 pre-route: schematic -> ERC/netlist -> A1 mechanical -> netlist import + placement -> fanout -> queued route
set -uo pipefail
cd "$1"; N=pcb-a-power
...
python3 ../tools/prefanout.py $N.kicad_pcb 'GND,+5V,CELL+' fine 2>&1 | grep -E 'fanout:'
