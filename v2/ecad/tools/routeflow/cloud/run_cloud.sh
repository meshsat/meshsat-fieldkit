...
# the EPYC core is slower than the VM's i9). Log ~/cloud.log, marker CLOUD-EXIT. Usage: run_cloud.sh <parallel per experiment> <experiment names or comma-joined groups...>
...
cd $R || exit 1
for g in "$@"; do   # a group "a,b" runs a then b in one background process (experiments that share a project directory must never run at once)
  ( for e in ${g//,/ }; do
  ...
  echo "started group $g (pid $!)"; sleep 8
done
wait; date
