...
for fn in sorted(glob.glob(os.path.join(out, "meshsat-1450-*.png"))):
    ...
    d.text((18, h + 8), "MeshSat field kit V2 concept in the Peli 1450 (catalogue 411 x 329 mm), the MESHSAT-830 board set   view: " + view, fill=(240, 240, 235), font=font)
    d.text((18, h + 42), "rulers in mm, 10 mm ticks, numerals every 50; floor grid 50 mm; prototype design, nothing built; " + os.environ.get("RENDER_DATE", "7 Sep 2026") + " (set " + os.environ.get("RENDER_SET", "eight") + ")", fill=(180, 180, 175), font=small)
    canvas.save(fn); n += 1
