...
apt-get update && apt-get install -y --no-install-recommends software-properties-common ca-certificates curl git python3 python3-numpy python3-scipy python3-yaml python3.12-venv xvfb xdotool x11-apps openjdk-25-jre rsync unzip zip procps || { echo "SETUP-FAILED apt"; exit 1; }
