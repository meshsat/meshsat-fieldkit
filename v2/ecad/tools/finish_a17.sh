#!/usr/bin/env bash
# A17: wait for route_parallel.sh (PARALLEL-DONE in the log), stub router, dangling clean-up, pack-node and boost bars, legend pass, finish.
...
python3 ../tools/fix_a17_node.py $N.kicad_pcb 2>&1 | grep -vE 'Debug|leak' | tail -4
python3 ../tools/silk_fix_all.py $N.kicad_pcb a 2>&1 | grep -vE 'Debug|leak' | tail -2
cd ..; ./tools/finish_board.sh pcb-a-power pcb-a-power post_fix_a.py meshsat-pcb-a-revA-A17 2>&1 | tail -16
echo FINISH-A17-DONE
