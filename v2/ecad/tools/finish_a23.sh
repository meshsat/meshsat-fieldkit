#!/usr/bin/env bash
# A23 (MESHSAT-862 rule 3, 8 Sep 2026), from finish_a22.sh: the same finish in the pcb-a-power-a23 copy: wait for route_parallel.sh (PARALLEL-DONE in the log), stub router, dangling clean-up, pack-node and boost bars, legend pass, finish.
...
cd "$1/pcb-a-power-a23"; N=pcb-a-power; LOG="$2"
rm -f out/a23-clean.txt out/par-score.txt out/contracts.log   # a stale clean flag must never finish a board (register class 6)
...
if [ "$PM" -ne 0 ]; then
  ...
  echo 'A23 PAIRS NOT MATCHED, not finishing (audit images in out/audit)'; echo open > out/a23-clean.txt; echo FINISH-A23-DONE; exit 1
fi
...
kicad-cli pcb drc --severity-all --format json -o out/$N-drc.json $N.kicad_pcb >/dev/null 2>&1
python3 ../tools/hardset.py out/$N-drc.json post --flag out/a23-clean.txt --label 'routed-board gate' | sed 's/^hardset:/routed-board DRC:/'
if ! python3 ../tools/check_pcb_a.py $N.kicad_pcb 2>/dev/null | grep -q 'RESULT: ALL PASS'; then echo 'A23 GATE FAIL on the routed board'; echo open > out/a23-clean.txt; fi
...
python3 ../tools/dc_drop.py $N.kicad_pcb --json out/$N-dc_drop.json > out/$N-dc_drop.log 2>&1; DC=$?; grep -E 'dc_drop' out/$N-dc_drop.log | tail -14; [ "$DC" -eq 0 ] || { echo 'A23 DC DROP MISSED or unresolved (out/$N-dc_drop.log)'; echo open > out/a23-clean.txt; }
python3 ../tools/stackup_write.py $N.kicad_pcb 2>&1 | tail -1   # the routed board carries the stack for the read-back (8 Sep 2026)
python3 ../tools/impedance_check.py $N.kicad_pcb --json out/$N-impedance.json > out/$N-impedance.log 2>&1; IM=$?; grep -E 'impedance' out/$N-impedance.log | tail -14; [ "$IM" -eq 0 ] || { echo 'A23 IMPEDANCE MISSED (out/$N-impedance.log; openEMS on the missed pairs)'; echo open > out/a23-clean.txt; }
# 8 Sep 2026 (MESHSAT-862): the cross-board contracts are part of every finish (they were called by no chain before)
python3 ../tools/check_contracts.py .. > out/contracts.log 2>&1; grep -E 'FAIL|MISSING' out/contracts.log | head -12; if grep -q 'ALL CONTRACTS PASS' out/contracts.log; then echo 'contracts: ALL PASS'; else echo 'contracts: FAIL (out/contracts.log)'; echo open > out/a23-clean.txt; fi
CLEAN=$(cat out/a23-clean.txt); if [ "$CLEAN" != clean ]; then echo 'A23 NOT CLEAN, not finishing'; echo FINISH-A23-DONE; exit 1; fi
cd ..; ./tools/finish_board.sh pcb-a-power-a23 pcb-a-power - meshsat-pcb-a-revA-A23 2>&1 | tail -16
[ "${PIPESTATUS[0]}" -eq 0 ] || { echo "A23: finish_board REFUSED the deliverable (verify_deliverable or an export step failed)"; echo FINISH-A23-DONE; exit 1; }   # 8 Sep 2026: a pipeline's status is tail's, so a refused deliverable used to be reported finished
echo FINISH-A23-DONE
