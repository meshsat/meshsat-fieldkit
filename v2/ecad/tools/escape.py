    ...
    for key, lst in sides.items():
        ...
        if FAN_OK and pitch <= FromMM(0.5) and len(lst) >= 2:
            ...
            for idx, (along, pad, L) in enumerate(lst):
                ...
                for lane, vpitch, depth, k in ([(0.75, 0, 0.8, 2), (0.75, 0, 0.8, 3), (0.35, 0, 0.7, 2), (0.35, 0, 0.7, 3), (0.35, 0, 0.55, 2), (0.35, 0, 0.55, 3)] if (0.8 - pitch / 1e6) * (len(lst) - 1) / 2 > 1.5 and (pitch - VIA_D / 2 - TW / 2) >= CLR + FromMM(0.05) else []   # and only where the stagger has margin: a neighbour's track passes a via at one pad pitch, so pitch - via radius - track half must clear the class clearance with room. At 0.4 mm with this branch's 0.45 via and 0.2 track that is 0.075 against 0.16 needed, which is what A23's charger QFN reported four times (8 Sep 2026 17:45); the 0.4 and 0.5 mm rows that do need a stagger are the ones the ROWS04 scheme above takes, with its 0.40 via and 0.127 track.) + [(0.75, 0.8, dp, 1) for dp in (1.3, 1.7, 2.1)] + [(0.35, 0.8, 1.3, 1), (0.35, 0.8, 1.7, 1), (0.35, 0.7, 1.2, 1), (0.35, 0.7, 1.0, 1), (0.35, 0.7, 0.85, 1), (0.35, 0.8, 2.1, 1)]:
