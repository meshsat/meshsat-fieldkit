...
python3 ../tools/gen_footprints_b16.py ../meshsat.pretty > out/gen_fp.log 2>&1 || { echo 'BLOCK footprint generator (out/gen_fp.log)' | tee out/preroute-gate.txt; tail -3 out/gen_fp.log; echo PREROUTE-DONE BLOCK; exit 1; }   # the SA868 footprint lives in the B16 footprint generator
...
python3 ../tools/gen_pcb_d.py $N.kicad_pcb 2>&1 | grep -E 'saved|WARN|Trace|Error|note'
