...
python3 ../tools/gen_sch_c.py $N.kicad_sch $N > out/gen_sch.log 2>&1 || { echo "BLOCK schematic generator (out/gen_sch.log)" | tee out/preroute-gate.txt; tail -3 out/gen_sch.log; echo PREROUTE-DONE BLOCK; exit 1; }; grep -E 'wrote|single-pin nets' out/gen_sch.log
