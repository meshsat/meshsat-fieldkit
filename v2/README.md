...
Peli 1450 without foam plus the 1450PF special application panel frame (CAD in `vendor/peli/1450/`; the 1520 files stay in `vendor/peli/1520/` for the record). The frame window is 349.65 x 233.83 mm; the face plate (365.5 x 249.5 x 3, ten M3 at the frame's inserts) sits with its top 101.4 mm above the cavity floor, the backer board 10 mm under it, and the stack tops out at 83 mm (the Compute Module cooler). The interior numbers, the battery row along the west wall, the nine bulkheads at 88 mm and the upright connector plate between the back-wall ribs are in design record 32.42; the templates in `release/revA/case/`. RF blind-mate (SMP-MAX) goes into this first build by the owner's ruling of 4 Sep 2026 (MESHSAT-775, design record 32.13); the boards in this folder still use SMA couplers on the wall strip until the PCB-A and dock respin lands.
