...
**Ruling in design: a connector plate.** Both receptacles mount on one flat aluminium plate, 82 x 54 x 3 mm, bolted over a window in the long wall on the dock's entry side with six M4 screws and a 2 mm closed-cell gasket. The plate is flat, so each receptacle seals on it with its own gasket, and the plate-to-wall gasket takes up the draft and the ribs. The 3 mm plate sits inside the 233-370's stated panel range of 1.6 to 6.35 mm. The case itself only needs two 29 mm hole-saw holes and six 4.5 mm holes.

**Positions (case frame, X along the long axis from the case centre, Z up from the cavity floor).** Plate centre (-92, 55), outline X -133 to -51, Z 28 to 82. Shore DC receptacle at (-110, 55), USB receptacle at (-74, 55), 36 mm apart. Plate screws at X -120, -92 and -64 in two rows at Z 32 and 78. Every through-hole keeps at least 7 mm from the nearest leg bosses (X -133.3 and -51.4, whichever long wall the dock side turns out to be, since the boss patterns of the two walls interleave); the plate bottom at Z 28 stays above the 16.7 mm floor chamfer; the plate top at Z 82 is 43 mm below the rim. The strip's `J_DCIN` is at case (-104, -106), so the shore lead is short.
...
**Open on the case, not on the drawing:** the outer skin between the ribs at the plate position has to be looked at on the case in hand before cutting, for latch or handle bosses that the envelope model does not carry. That is a look, not a measurement.
