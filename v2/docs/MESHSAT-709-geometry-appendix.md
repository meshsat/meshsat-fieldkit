...
- *Antenna paths (02:20, found while building the concept renders).* The NEO-M9N on B13 needs a bulkhead: it takes the third RF path (A20 site 3 at (-26, -56), west end wall +30), which had been the second WiFi site; the CM5's antenna is dual-band and uses the WiFi 2.4 path alone; the east +60 site stays the plugged spare. `case_wall_cutouts.py` and the A20 site label say GNSS now (the silk text lands on the next A regeneration; the copper is unchanged).
