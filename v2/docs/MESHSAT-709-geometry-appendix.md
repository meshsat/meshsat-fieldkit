...
- **Boost feeds** (`BOOST`): the Pi converter draws up to 9.5 A at a 3.0 V node and JLC's 4-layer stack has 0.5 oz inner copper (a 6 mm In2 column alone is good for about 3 A at 10 K), so each feed is an In2 column from the blade fuse's BOOST pad (pin 2, x 5/30/55, plated through, so the column starts in the pad itself) north to the inductor row, widened to a 19 mm foot where it passes under the CELL+ node bar (y 148.5 to 152, between the CELL+ pad rings of the neighbouring fuses), plus a B.Cu column of the same width in parallel from just north of the node bar to the inductor jog, tied to the In2 column by four locked 0.8/0.4 vias at its south end and by six vias in a top-side tap island over the south end of each inductor's pad 1 at its north end (about 11 A at 10 K for the pair). Run 9's B.Cu feed bands had been centred on the fuses' CELL+ pads (pin 1) and were cut by the node bar and by the M2 collector; the boost 1 column starts 0.5 mm east of U22's BST1 escape via after run 10 left that net open with the keep-out edge touching the via.
- **Track keep-outs**: every collector, riser, column, foot and jog carries a rule area on its layer that forbids tracks and permits vias (the two rail taps carry none: they sit in the capacitor columns whose GND vias need the room, and run 13 left BOOST_EN open when it could not via past the M2 tap) (`track_keepout()`; KiCad exports it as a `wire_keepout`), so the router crosses a band on another layer and cannot slice it; `escape.py` and `prefanout.py` ignore a rule area that permits vias. Two router facts learnt on the way: exporting the bands as DSN planes (run 8) made the router end tracks at band edges the fill never reached, so `route_one.sh` strips every zone from the DSN again; and Freerouting places no via of its own inside a wire keep-out, so a pad under a band whose net must leave on an inner layer needs the band moved or a pre-placed via (R18, BST1).
