...
**Power.** The card asks 3.3 V at up to 3 A (average 4 to 8 W, maximum 9 W): a third TPS563201 `U36` (+3V3_WIFI) on rail M2 through its own 50 mOhm shunt and INA219 `U35` at 0x45 (the map of 32.35 has 0x45 free), enabled directly by the module's `PCIE_PWR_EN` (3.3 V, active high, 100k pull-down), so the card rail follows the module and its PCIe link state as on the CM5IO (which does the same with an AP3441 buck). Rail M2 (SDR channel, RockBLOCK channel) gains up to 1.8 A at 5 V; the A20 TPS61288L rails were sized for 5 A class loads (32.21, the Pi rail proves the coil and switch at 5 A from 3.0 V), so M2 stays within the same converter design; the rail lead is the same 18 AWG VH pair. Recorded as a budget note, no A20 change.
