...
**A21, what the class widths alone could not do (5 Sep 2026 14:15 to 14:45, runs 6 to 9 on the VM).** Applying the classes as written (RAIL 2.0 mm, BANK 1.0) left 20 connections open; at 1.0 and 0.6 mm six stayed open, every one at pin 5 of U22, U23 and U24: the TPS61288L's VOUT pin is a 1.25 x 0.40 mm pad with pad 4 and pad 6 0.25 mm away, so no track wider than about 0.5 mm can leave it, and the escape scheme had given each rail one 0.2 mm stub and one 0.45/0.25 via as its whole current path out of the converter. The rail current therefore does not travel in router tracks at all on A21:

- **Top-side output islands** (`gen_pcb_a3.py`, `island()`): per converter a 0.5 mm neck over the east end of pad 5 (between pad 6 and the package edge) that widens east of the escape vias into a 14 x 19 mm F.Cu pour over the output capacitor group (full pad connection, 0.15 mm clearance, priority 3), so the VOUT pad, the six output capacitors and the escape via sit in one piece of copper.
- **Stitch vias** (`stitch()`): six locked 0.8/0.4 vias per island into the rail's bottom band (M2 into its collector, PI into a tap) or into the In2 plane (M1, in the column between the converter and its capacitors, clear of the M2 riser).
- **Bottom bands** (`BANDS`): M2 collector 37 x 6.5 mm at KiCad y 122.0 to 128.5 (widened so U23's escape via row at y 127.25 leaves 4.7 mm) and its 6 mm riser to the VH connector; the PI rail runs north of the boost row (collector 54.5 x 3.3 mm at y 109.5 to 112.8, tap 9 x 14.5 mm east of the boost 3 inductor band, riser 6 mm at x 25 to 31), because run 8 showed the first PI collector at y 119.5 to 124 cut by the boost 3 inductor band and its riser cut by the boost 2 inductor band (same layer, other net). The boost bands keep their run 7 outlines. Same-net bands overlap only at distinct priorities (`check_zone_nets.py`).
- **Track keep-outs**: every band carries a B.Cu rule area that forbids tracks and permits vias (`track_keepout()`), so the router crosses a band on F.Cu, In1 or In2 and cannot slice it; `escape.py` and `prefanout.py` ignore a rule area that permits vias. Run 8 had tried the other way, exporting the bands as DSN planes: the router then ended tracks at band edges the fill never reached (BOOST and CELL+ opened), so `route_one.sh` strips every zone from the DSN again.
- **Classes**: RAIL is the link width only, 0.5 mm (via 1.0/0.5); BANK 0.6, BOOST 0.8. `check_pcb_a.py` reads the routed widths of the unlocked tracks against those floors, and on a filled board it also requires every rail band, boost band and island to be one piece over at least half its outline and every M2 and PI stitch via to sit in a filled rail band; `finish_a21.sh` refills, runs that gate, and refuses to finish or commit with an open connection, a hard violation or a FAIL.
