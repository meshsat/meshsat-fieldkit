...
engineering items; the lid sensor is drawn on board E since `faf8c981` (section 4, Reduced row). Whether the kit is powered at all in transit is
...
| EMCON | `SW_EMCON` closed (locking toggle) | toggle opened | receive-only radios (GNSS, DCF77, lightning sensor), the VHF receiver, compute, display in its lighting mode | radios dark (owner ruling D-05), as section 4b: power removed from the SDR, the RockBLOCK, the LoRa module, both E72, the HF unit and the two WiFi link cards; the compute modules' own WiFi and Bluetooth disabled through open drains; the 5G module in airplane mode through its disable pin; the PA rail and keying off; software holds every send, an SOS included | HW for the gated rails, the module radio disables and the VHF keying; the 5G row rests on the module's firmware until its supply removal is drawn; SW hold on top | PS-EMCON 47.6 W, PROVISIONAL, computed for the circuit before `458b2873` (section 4a) | `PANEL.md` sections 6 and 9; `feasibility/EMCON.md`; `gen_sch_b.py` lines 478, 716 to 741, 974 to 977 and 1029, `gen_sch_a.py` line 1102, `gen_sch_d.py` lines 532 to 534, at `45bde541` | session work owed under D-05 (S-01): the 5G module's staged supply removal (SD-EMC-1), the line's shared items (`feasibility/EMCON.md` section 7) and the back-feed paths (SD-EMC-2); a hardware EMCON lamp on board C (SD-EMC-6); twelve bench tests, none of them with the kit's own SDR (EMCON.md section 6). **Corrected 26 September 2026 (S-07):** this row said the module radios were off the line and the WiFi cards had only their disable pin, the circuit before `458b2873` |
...
| PS-EMCON | EMCON as generated at `82dd1e4d`: PS-TYP with the radios then gated off (the dark meaning D-05 rules); since `458b2873` EMCON also removes the WiFi link cards' supplies and the module radios, which lowers the figure by an amount not computed | 47.6 | 2.8 / 2.3 | 3.8 / 3.0 |
...
cluster-idle definition) kept until the session defines the mode; EMCON is PS-EMCON (the D-05 fixes of `458b2873` lower it
by an amount not computed); Charging, Blackout and NVG are overlays; Startup, Shutdown,
...
| SA868 VHF with the 30 W PA | the KEY gate on board D (`KEY = PTT_ANY AND TX_INHIBIT_n`) and the PA rail and keying on board A (`PA_EN = EMCON_HW AND PA_SW_EN`, `U26`); the exciter's supply is not gated and the resting relay joins antenna to exciter | transmit only | continues |
| QMX HF | the enable of the converter that feeds its DC input (`HF_EN = EMCON_HW AND HF_SW_EN`, `U26`, `gen_sch_a.py` line 1102); its USB supply is not gated | power (its receiver runs from the DC input per its manual) | lost |
...
switch is SD-EMC-1's and is owed. **Corrected 26 September 2026 (S-07):** this section described the circuit before
...
outlets at their minimum contract. The session set both thresholds as PROVISIONAL figures,
not pass lines (session choice SC-10 of the requirements registry: every transmitter at once only above a pack rest
voltage of 15.5 V, each PA key-down at most 60 s; `feasibility/POWER-THERMAL.md` section 7.2), and they stay **TBD**
as pass lines. The hardware interlock that drops the outlets while the PA keys, which the design record asks for
