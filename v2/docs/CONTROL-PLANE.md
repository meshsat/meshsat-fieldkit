...
| 3 | a board routes end to end from one job spec, the report regenerates, `ledger_verify` passes | **P3 did exactly that**, and `ledger_verify` on the box re-walked **9 real chains, 510 rows, 0 problems**; on the runner, which has no ledger, it is INCONCLUSIVE and never a pass |
