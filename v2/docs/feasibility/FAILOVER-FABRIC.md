...
|---|---|---|
| **Board B round-6 candidate netlist** | `scratchpad/wt/r4b/drafts/box/r6-run6/tint/regen/pcb-b-compute.net`, sha256 `af8a9186f981210c48104eaf5742fb4cd4f80d12aa0f13bc5d887e75c922a380`; written by `gen_sch_b.py` sha256 `6e3d880901fd30306c0fa8bda35a86da6450d7e4bb15217f9cc3225da8d5faef` (uncommitted in worktree `fnd/r4b` on base `82dd1e4d`, applied to `main` `faf8c981` on the box, run 6, 26 Sep 2026 10:51 to 11:33 UTC); the `t6` regeneration of the same generator is PARITY_AFTER_NOISE with it (content hash `142fe788dabbaf58`, `r6-run6/parity_netlist_t6_tint.json`). It is byte for byte the file the round-6 integration set (O-18) commits as `v2/ecad/pcb-b-compute-b19/out/pcb-b-compute.net` (`r6-run6/tint/set.sha256`) | every row of the map |
...
| Board A netlists | A23 on `main`: `v2/ecad/pcb-a-power-a23/out/pcb-a-power.net`, sha256 `aa1372ba3932610f...`; round-6 candidate: `scratchpad/wt/r4a/drafts/box/fixup3-run2/repo/pcb-a-power.net`, sha256 `daf132b22fad9854...` (`gen_sch_a.py` sha256 `17c204ad4fb0cddb...`) | cross-board pins and enables (section 6) |
...
| Round-6 author records | `scratchpad/wt/r4b/drafts/r4-decisions.md` (sha256 `27023b76...`), `r4-interfaces.md` (`dafa398f...`), `drafts/box/r6-run6/` (README, run log, gate emulation) | what the candidate changed and why; open items O-01 to O-24 |
...
| [XOR-TI] | TI SN74LVC86A, SCAS288R, revised August 2024. **Fetched by this session**: https://www.ti.com/lit/ds/symlink/sn74lvc86a.pdf, 26 Sep 2026 12:31 UTC | `drafts/datasheets/ti-sn74lvc86a.pdf`, `8115c852d23ce42a0a2518462474171d809c6d6ad5ed28f21c110bdb5a60a334` |
| [XOR-NX] | Nexperia 74LVC86A, Rev. 8, 25 January 2024. **Fetched by this session**: https://assets.nexperia.com/documents/data-sheet/74LVC86A.pdf, 26 Sep 2026 12:31 UTC | `drafts/datasheets/nexperia-74LVC86A.pdf`, `aa449c7949d141d555967710a235c7686a6ece02468f783f8c8cf924a1fc926a` |

**How the map was read.** `drafts/fab/fabmap.py` (sha256 `5b2bb0cf...`, with `tracer.py` `b624e4f6...` and
...
(`out/pulldowns-mutations.txt`, `7510bd67...`). All read-only; they are the session's working files (worktree
`fnd/rv-fab`, `drafts/fab/`), not repository tools. The root rule `out/` (`.gitignore:20`) would drop the cited
outputs from any commit of `drafts/`; `drafts/fab/.gitignore` (`!out/`) re-includes them, checked with
`git check-ignore`. The H753 datasheet's text sits beside the H743's in `drafts/fab/txt/`.
...
| Supervisor alternate functions | [MCU] Tables 11, 13 and [MCU-53] Tables 10, 12; pin identity 100 of 100 on both H743 and H753 (round-6 `drafts/pin_parity.py`, RECORDED) | VERIFIED |
| Crystals (hubs, KSZ, supervisors) | `clock_check` PASS of 7 on the round-6 regeneration (`r6-run6/run.log`, RECORDED) | RECORDED |
...
Generated by `render.py` from `fabmap.json` (both in `drafts/fab/` of worktree `fnd/rv-fab`), on the candidate netlist
