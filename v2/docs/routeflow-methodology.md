...
3. **Routed**: a session file exists for the winner; `hard 0 of 15 types` (the one set of `hardset.py`; until 10 September 2026 the supervisor said six here and the finish refused on fifteen), `unrouted 0 of N nets` on the filled board; pairs within 1 mm.
...
- Finding (6 Sep 2026, 09:00 to 14:20): a Freerouting settings block costs the design's clearances on the dense board. The same B15 DSN routes at 0 hard with no `-dr` file and no `autoroute_settings` block (the production run of 04:41), at 42 hard (17 shorts, 16 clearance faults, a track along the board edge) with a "default" block passed as a `-dr` file, and at 42 hard with the identical block written into the DSN's structure; the block without its layer lines gives 10 hard; a block with only some of its items (`fr_rules.py --only`) switches the autorouter off (4 vias, 110 to 325 opens). So every sweep row of the night carries a settings block and compares knobs against knobs, never against production; the Stage 4 A/B is rerun with `plain` rows (no block) beside the block rows, and the S1 knobs stay unmeasurable on 1.9.0 until the block's effect on the clearance classes is understood (the 2.4.1 command-line knobs are the next path). Rule: a "default" configuration is proven equal to production by a byte-identical result before anything is swept from it.
- Rendering (6 Sep 2026, 13:14 and 14:11): the first 1450 set (43 views, 13 minutes on eight H100 PCIe at 256 samples, Cycles on OptiX) was rejected: the closed lid sat 25.4 mm too low (a strap offset of the 1520 lid model; the 1451-931 lid mesh spans 0 to 70.9 from its own rim), which put the antenna sockets above the parting line, the scene stood invented whips on every socket, and the set showed the inside in three views of 43. The second set draws the sockets as jacks with the two antenna forms the record names, hides each board's parts with it in the level views, and spends 27 of 39 views inside. Previews at RESPCT=40 SAMPLES=48 run on the CPU box's RTX 3060 Ti in a minute per view and are looked at before the H100 box is rented; the key goes onto the account and into the onstart before creation (a key attached after the container starts lands with wrong file modes and sshd refuses it).
