...
1. **WiFi and 5G on USB3, or WiFi duplicated on PCIe.** Moving both radios to USB deletes the three PI7C9X2G404SL switches and frees roughly 1200 to 1800 mm2, which is most of the area the control plane needs. It is gated: a USB3 WiFi part enters the design only when peer-to-peer operation with no access point (802.11s or IBSS) is confirmed from the module maker's own documentation, per the owner's condition. The MT7915 in the currently ruled AW7915-AED is documented for mesh; the fallback if the gate fails is to keep it on PCIe and duplicate it on a second slot. The RM520N-GL's USB 3.0 mode needs the same confirmation from Quectel's hardware design guide.
2. **One board or two.** If the PCIe switches stay, the control plane needs its own board and a low-speed ribbon, whose loss degrades to the fail-safe home assignment.
3. **B18 is superseded.** Its information route remains useful as placement data; no B release is cut against the current topology.
