...
3. **OPEN: whether the WiFi card is duplicated.** With WiFi on PCIe it is slot-bound, so the kit-to-kit link dies with its module unless a second card sits on another slot. That is high availability by duplication, the same answer as the NVMe.
