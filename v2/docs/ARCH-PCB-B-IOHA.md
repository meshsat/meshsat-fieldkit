...
**A transformer-less link carries one requirement that is not copper, and it binds the software (17 September 2026, decision 29).** Microchip's own clause 6.6 (`v2/vendor/cluster/ksz989x-hw-design-checklist.pdf`, DS00004151A page 12) permits ports 1 to 3 to be coupled with a single 0.1 uF per signal inside one PCB, which is exactly what is built, and it ends with: *keep auto-negotiation enabled when 1000M speed is used*. **So the bridge, the OS image and any switch configuration written for this kit must never force a fixed speed or duplex on ports 1 to 3.** A forced link there stops working for a reason no gate in this project can see and no log on the board will explain. Port 4 is behind magnetics and is not bound by this. The other half of that clause, whether the module's own PHY needs termination when the transformer is removed, was decision 29 and is RULED (the session's, 21 September 2026): the links stay capacitively coupled as built, because the only document describing this topology permits exactly what is built, the compute module's datasheet says 'connect to transformer or MagJack' of pins designed for a cable port and nothing of these three links leaves the enclosure, and the module's PHY datasheet is not published so no wait produces it. The residual risk is that the three inter-module links do not come up, and it is discovered by the bring-up test below, not in the field. **So the auto-negotiation requirement in this paragraph is not advisory any more: it is the condition the ruling rests on.**
...
2. **Fail-safe defaults.** With all three controllers dark the voter outputs pull to *home host* and `OE` pulls to *disconnected*. A dead control plane degrades to the static assignment the board has today, not to contention.
   **Corrected 26 September 2026 (`v2/docs/feasibility/FAILOVER-FABRIC.md`, section 9, documentation items and FAB-04): the generated board does not do this.** With the plane dark every voted select reads its pull-down, so each bank's muxes select the home host, and each bank's `BOE{b}_n` (the XOR of `BSEL{b}` and its delayed copy) sits low, which ENABLES the muxes: each bank rests connected to its home module, the behaviour FMEA rows 4 and 20 and test A12 describe, not disconnected. And the 100 kOhm pull-downs on all 21 controller outputs do not hold a guaranteed low at the parts' datasheet leakage (2.025 V against VIL 0.8 V); the remedy taken is 10 kOhm (FAB-04, owed on board B).
3. **Break before make.** The voted `OE` is de-asserted, a delay elapses, the voted `SEL` changes, `OE` is re-asserted. A hardware delay element prevents `OE` rising coincident with a `SEL` change, so the ordering is not a firmware promise.
   **Corrected 26 September 2026 (`FAILOVER-FABRIC.md` FAB-03): the generated circuit does not have this order.** The voted `BSEL{b}` drives the muxes' `SEL` directly, and `BOE{b}_n` rises one XOR delay later (at most 4.6 ns), so the select moves before the enable drops and nothing in the parts' documents guarantees that the old path is off before the new one conducts; the RC-delayed copy also reaches the XOR slower than its input rule allows. The fix taken is a true sequence in hardware (the select from a Schmitt-buffered first delay, the enable from the XOR of the select and a longer Schmitt-buffered delay), owed on board B; test A10 is kept as written.
4. **The hub's own rule, where a USB4715 front end is used.** Microchip's AN2341 states that with FlexConnect *"only one of these hosts may have access to the USB tree at a time"*, and the hub **auto-reverts to its default host when VBUS_DET on the owning port goes low**. Two hosts cannot be attached, and the commonest failure case, a module losing power, needs no controller at all.
   **Corrected 26 September 2026 (`FAILOVER-FABRIC.md` section 9, documentation items, and FAB-02): this mechanism is not in the design.** The hubs are TI TUSB8041 with `USB_VBUS` on `+5V_DEV`, not USB4715s, so nothing detaches a bank from a host that has lost power, and the always-on fabric applies voltage to the pins of an unpowered module. The fix taken qualifies each bank's enable, and the display switches' enable, with the selected slot's power-good, owed on board B together with FAB-03's rework.
...
1.3 ns of line against a 200 ns bit), with a split termination of 120.8 ohm at each physical end doing the work.
**Corrected 26 September 2026 (`FAILOVER-FABRIC.md` FAB-05):** the fitted TCAN334D is specified to 1 Mbps (the
TCAN334G is the 5 Mbps part, with the same pins), so the fabrics run at 1 Mbps or less, a supervisor firmware contract;
at 1 Mbps the bit is 1000 ns and the reason for leaving the pairs out of an impedance class holds more strongly. What
...
**Seven voters**, one per voted bit (the three bank selects, the three hub resets and the WiFi changeover
`WIFI_SEC`), `out = AB + BC + CA` in 74LVC08 and 74LVC32 quads, the same families the EMCON chain already uses. Verified
on bank 1: the three controllers' selects enter U70 at pins 1/9, 2/4 and 10/5, the three products leave at 3, 6 and 8,
and the OR tree in U76 produces `BSEL1`, which reaches both muxes, its pull-down, the RC and the break-before-make
detector. Every one of the 21 controller outputs (seven voted bits times three controllers, R480 to R500) carries a
pull-down, so a controller that is absent, unpowered or in reset is read as a definite no rather than as an undefined
CMOS input. **Corrected 26 September 2026 (`FAILOVER-FABRIC.md` section 4.9 and FAB-04):** this paragraph said six
voters and eighteen outputs, which predates `WIFI_SEC`; all seven evaluate as 2-of-3 majorities on the netlists; and at
100 kOhm the pull-downs do not hold a guaranteed low at the parts' datasheet leakage, so the "definite no" holds at
typical leakage only until the remedy (10 kOhm) is in the generator.
...
**Corrected 26 September 2026 (`FAILOVER-FABRIC.md` FAB-03):** the select reaches the muxes before the enable drops, so
this detector does not break before it makes; section 5 item 3 states the fix.
...
6. **RULED, 21 September 2026 (decision 29, the session's): the three module links stay capacitively coupled and no magnetics are fitted.** The reading that decided it is the compute module's own pinout, Table 4, where all eight Ethernet pins read 'connect to transformer or MagJack', which is the cable application and not a prohibition, beside the switch vendor's clause 6.6 permitting exactly what is built for a PHY-to-PHY link inside one PCB. The fallback is declined with a cost the question did not know when it was asked: the H5007NL is a 0 to +70 C part against a -20 to +40 C envelope. What follows is the record as it stood when the decision was open. The switch vendor permits it and three of its four conditions are met by the copper conductor by conductor; the fourth sends the reader to the other device's documentation, and the other device is the module's Broadcom PHY, whose datasheet is not published. The module's maker wires their own carrier through a MagJack with no series capacitor and no termination network (`v2/vendor/cm5/cm5io-kicad.zip`), so their reference answers nothing about the transformer-less case. The question to put to them is one sentence, and the fallback is three more of the H5007NL this board already carries: 5.26 USD and 982 mm2. The numbers are in `tools/pcb_decisions.yaml`.
