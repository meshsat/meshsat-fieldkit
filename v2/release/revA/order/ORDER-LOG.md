...
## 4 Sep 2026 night: A19 pass 5, the sealed panel C5, the spacer ring R1 retired (appendix 32.33, 32.34)

PCB-A A19 was rerouted on locked fine-pitch escapes (0 hard, 0 unrouted); `PCB-A-POWER-A19/` is rebuilt from it, `PCB-E5-BLOCK-E5/` from the rebuilt block. The control panel is respun as **C5**, the case's sealed weather face: `PCB-C-DISPLAY-C5/` replaces `PCB-C-DISPLAY-C4/` (now under `superseded/`), and `PCB-C-RING-R1/` moves under `superseded/` with no replacement. What changes for the cart when the owner declares the set final: line Y20 (C4 PCB and PCBA) is deleted and rebuilt from `PCB-C-DISPLAY-C5/` with the via covering set to **plugged** (JLCPCB plugs holes up to 0.5 mm; every via on C5 is 0.3 or 0.4) and the outline's four keyed switch holes (three 2.70 x 1.10 keyway notches and one D flat, routed over the drilled holes); the assembly has fewer through-hole rows (the sounder and every connector left the JLC BOM or became SMD on the underside); line Y21 (R1, EUR 21.75) is deleted. The die-cut seal outlines (`pcb-c-display-seals.dxf` in the deliverable folder) are not a JLC file. No cart edit was made (owner, 4 and 4 Sep: the cart is rebuilt once, when the whole set is final).

## 5. Order set rebuilt for the MESHSAT-830 board set, 8 Sep 2026 04:25 CEST (runner session, for the EasyEDA/JLCPCB sponsorship quote, MESHSAT-776)

The seven upload folders are now A22, B16, C7, D8, E6, P1, E5 (A21, B15, C6, D7, E4 moved to `superseded/`). **`PCB-B-COMPUTE-B16/` is QUOTE ONLY:** B16 is held on its placement and has no routed deliverable, so its line is exported from the placed, unrouted pre-route board (`boards/meshsat-pcb-b-revA-B16-quote/`, `QUOTE-ONLY.txt`); size, six-layer stack, finish and the parts list are the design's, so the price is right; never pay that line, it is rebuilt from the routed board (B17) before any payment. Its BOM has 79 of 173 lines without an LCSC code (matched on the site at quote time). The old cart lines Y18 to Y24 (the 1520-era set) are to be deleted before the new lines go in. Purpose of this pass: a cart screenshot of the whole current set with the summary box visible, for EasyEDA's sponsorship process (Dom Qin, 8 Sep); nothing is paid.

## 5.1 JLCPCB quote cart of the MESHSAT-830 set, 8 Sep 2026 12:25 CEST (ordering session on nllei01gpu01, MESHSAT-776; nothing paid)

Quote for the EasyEDA sponsorship request (Dom Qin). The laptop session (ankh, 04:30 to 05:40 CEST) deleted the 1520-era lines Y18 to Y24, uploaded the seven boards as file cards under project MeshSat (two rounds; the second round of 05:08 to 05:10 carries the LCSC-coded BOMs from `<folder>/upload/<stem>-bom-jlc.csv`) and put A22 (Y25) and B16 (Y26) in the cart. This session quoted C7, D8, E6, P1 and E5 from the second-round cards and saved them as Y27 to Y31. Every line: qty 5, 1.6 mm, ENIG, matte black, white silk, remove order number, Standard PCBA (Economic greyed out), edge rails by JLCPCB, Confirm Production file and Confirm Parts Placement on with "Do not confirm automatically" ticked, product description Research\Education\DIY\Entertainment > Development Board HS 847330. **Impedance control was not ordered on any board.** B16 (Y26) is QUOTE ONLY (exported from the placed, unrouted board): never pay that line.

| Board | PCB line | PCB EUR | PCBA line | PCBA EUR | Sides | BOM lines: confirmed / shortage / not placed | Matching notes |
|---|---|---|---|---|---|---|---|
| A22 POWER (6L, 3313 stack, 0.25 via) | Y25-11651261A, 8-9 days (replaced by Y32 at 12:44, see the addendum) | 135.25 | SMT026090860345, top, 5-6 days (Y32: SMT026090861766, 1131.74) | 1131.75 | top | 129: 127 / 1 (C1,C2 100u 25V C2911627 0 stock) / 1 unmatched (C11 group 22u 50V X7R 1210 C55533070 pre-order) | laptop session; at 12:11 the cart flagged its L1 XAL1010-103MED C6358489 as out of stock (1 left) and greys the two Y25 lines out of the selection; see the A22 note below |
| B16 COMPUTE, QUOTE ONLY (6L, 3313, 0.20 via) | Y26-11651261A, 8-9 days | 163.20 | SMT026090860387, both, 5-6 days | 1814.06 | both | 173: 171 / 2 (U13,U14 E72-2G4M20S1E C5352930 0 stock) / 0 | laptop session; site-side replacements in the B16 list below |
| C7 PANEL BACKER (4L, 7628, 0.20 via) | Y27-11651261A, 3 days | 152.12 | SMT026090861435, both, 3-4 days | 203.08 | both (36 top, 92 bottom) | 57: 41 / 0 / 16 (D1 to D16, the 3 mm face LEDs, bench-soldered, Do not place) | D17 C2166 (an LED) to C7502705 BAT54W; U1,U2 C50993 (not found) to C2864778 PCA9555PWR; R1,R6,R37 1k C11702 (0402 part on the 0603 land) to C21190; R18 47R C25118 (0402 part) to C23182 |
| D8 APRS (4L, 7628, 0.25 via) | Y28-11651261A, 3 days | 61.16 | SMT026090861512, both, 5-6 days | 251.73 | both (88 top, 92 bottom) | 66: 60 / 1 (C58,C60 22p 500V NP0 1206 C2840256: 14 in stock against 15, no stocked alternative) / 5 (L1,L2 68 nH 1812; U4 TUSB2046BI LQFP-32; Y1,Y2 6 MHz 3225: no JLC part) | U16 C5626 (a 74HC245PW) to C2864778 PCA9555PWR; LED1,LED3,LED6 green C72043 (1 in stock) to C2986059 |
| E6 DOCK STRIP (4L, 7628, 0.20 via) | Y29-11651261A, 3 days | 91.29 | SMT026090861587, top, 6-7 days | 419.77 | top (138) | 87: 86 / 0 / 1 (U5 LT8705AIFE C674167, 1 in stock, its BOM comment says bench-fitted) | LED1 green C72043 (19 short) to C2986059; D7,D8 SS14 C51897884 and J_LTG 1x5 header C55629812 were not auto-accepted and were matched by hand to the same codes; the site notes C55629812 has no footprint model yet (placeholder preview, one extra PCBA build day) |
| P1 PACK BMS (2L, 2 oz) | Y30-11651261A, 24 h | 48.55 | SMT026090861627, top, 4-5 days | 75.69 | top (42) | 19: 18 / 1 (F1 Keystone 3568 holder C5249699, 2 in stock against 5, Do not place, bench-fitted) / 0 | Q1,Q2 C2825431 (a lever switch, 0 stock) to C529279 CSD17570Q5B (112 in stock) |
| E5 DOCK BLOCK (2L, 2 oz, bare) | Y31-11651261A, 3 days | 47.28 | none | 0 | none | none | deburring/edge rounding left at the site default Yes (0.09) |
| **Sum of the 13 lines** | | **698.85** | | **3896.08** | | | **4594.93** |

Cart summary read 12:16 CEST with every selectable line ticked: 11 items selected (the seven PCB lines and the four PCBA lines other than A22; the site refuses to select the A22 PCBA line while it carries an out-of-stock part, and its PCB line follows), merchandise total **3327.91 EUR**, shipping estimated **105.23 EUR** (FedEx, est. shipping date 2026-09-17, 9.86 kg), subtotal **3433.13 EUR**. Adding the two A22 lines (1267.00) the merchandise total of the whole set is **4594.91 EUR** before shipping, VAT and coupons (the line prices sum to 4594.93; JLC rounds). Screenshot: `cart-830-set-2026-09-08.jpg` (this folder) and `~/Downloads/jlcpcb-cart-830-set-2026-09-08.png` on nllei01gpu01; per-board quote and placement screenshots in `~/Downloads/jlcpcb-quote-2026-09-08/shots/` on that host. The estimate given to Dom (3,000 to 3,500 EUR) is under the number: the two large six-layer boards alone are 3,244.25.

**A22 line and the stock change (12:11 to 12:20 CEST):** the cart shows "[C6358489] out of stock. Please go Select Parts page to find alternatives" on Y25 (XAL1010-103MED, L1; it had stock at 05:38). Its Select Parts page was opened and L1 matched to XAL1010-153MED C2802562 (15 uH stand-in, 1538 in stock, same price class; already the L10 stand-in), U26 C5605 (a 74HC14D) to C465737 SN74LVC08APWR and U27,U28 C5626 to C2864778 PCA9555PWR (127 of 127 confirmed), but the re-opened line quoted its PCB at 304.50 instead of 135.25 with 169.26 not itemised (the build time showed 2-3 days where the cart line says 8-9), so it was NOT saved: Y25 stands as the laptop session left it. To make it selectable, re-open Select Parts on Y25, redo those three matches and save only if the PCB price reads 135.25, or rebuild the line from the A22 card (the upload CSV now carries the corrected codes).

**Old lines:** Y18 to Y24 (the 3 Sep set, 1180.40 EUR: A16, B10, C4, R1, D5, E1, E2) were deleted by the laptop session before the upload; their record is in `~/Downloads/jlcpcb-quote-2026-09-08/old-cart.txt` on ankh and in section 6.1 above.

**Site facts of this pass:** the C7 card did not auto-detect layers or size (form stayed at 2 layer, empty size after "upload finished"); 4 layers and 344.1 x 228.1 were set by hand, then the JLC04161H-7628 stackup and the 0.2 mm via (which adds the Kelvin test and TG155). The first NEXT on the C7 card's BOM gave "File processing failed, please check your file and upload again"; a reload of the smt-order URL processed it fine. Rows the site merges onto one part come unticked with qty 0, and the merged group's sub-rows (rowspan cells) are separate table rows that need their own tick: on E6 twelve such rows were missed by the first pass and caught by a second (script over every unticked checkbox whose row carries a C-code). The first "Do not place" answer after NEXT did not advance the step on E6 and P1; NEXT again and the same answer did. A "Confirm Production file" dialog closed once without the tick registering (D8); redone with No, Yes and a zoom on the box before Confirm. One Order Now tab (E6) opened without the session (Sign In, USD, empty cart); a second Order Now from the Files tab was fine. P1's SAVE TO CART returned "Network Error" then twice "System error" but the first attempt had created Y30. A cart reload once rendered "Your Cart is Empty" with the badge at 7; the next reload showed all lines.

**B16 site-side replacements (laptop session, from `b16-site-fixes.txt`):** J_LIME C2802372 (a TVS diode) to C5355286 Wuerth 692122030100; J_M2C1 C2803764 (a switch) to C41430829 HYCW16A-05NGFF-420B (A-key stand-in for the TE 2199230-4 E-key); J_M2C2 C2803770 (a rotary switch) to C41430835 HYCW01B-05NGFF-420B (B-key stand-in for TE 1-2199119-5); J_M2C3,J_M2N1..3 C2803771 (a rotary switch) to C41430851 HYCW25M-05NGFF-230B (M-key stand-in for Amphenol MDT420M02001); LED1..LED36 green C72043 (1 in stock) to C2986059; Q1 C2827865 (no such part) to C455160 FDMC3612; T1 C2891392 (no such part) to C6384935 Pulse H5007NL; U3,U4 C2846591 to C157482 TS3DV642A0RUAR; U10 C2842210 to C699536 TMP117AIDRVR; U11 C22453086 to C29781241 Quectel LG290P03AAMD (78 in stock, 86.22 each); U12 C2913441 (a film capacitor) to C411294 E22-900M30S; U13,U14 C5352930 E72-2G4M20S1E correct but 0 stock (10-piece shortfall); U19,U20 C6009 (74LCX126, 0 stock) to C465737; U101,U201,U301 C2827826 (a coax connector) to C500767 PI7C9X2G404SLBFDEX; U102,U202,U302 C2841998 to C544686 TUSB8041IRGCR; J_DBG1..3, J_ZBDBG1..2 1x5 header C55629812 matched by hand. The B16 line still prices U1 KSZ9897 under C2830106 and U5 TPS23861 under C2927919 (FM2118, 10 in stock): the correct codes are C638299 KSZ9897RTXI and C93245 TPS23861PWR.

**Design LCSC codes found wrong at JLC and written back into the generators (owner ruling 8 Sep 2026 about 11:40 CEST: "edit the generators yourself, only the LCSC strings"; 33 literals in `gen_sch_a.py`, `gen_sch_b.py`, `gen_sch_c.py`, `gen_sch_p.py`, `lcsc_fill.py`, all compile; the six `upload/*-bom-jlc.csv` carry the same map):** PCA9555 C5626 (a 74HC245PW; A22 U27,U28; B16 U6,U7; the `lcsc_fill.py` table, so D8 U16 too) to C2864778; PCA9555 C50993 (not found; C7 U1,U2) to C2864778; 74LVC08 C5605 (a 74HC14D; A22 U26) to C465737; green 0603 LED C72043 (1 in stock; the table, so B16, C7 D18 is a different part, D8, E6) to C2986059; BAT54 C2166 (an LED; C7 D17) to C7502705; C7 R1,R6,R37 1k C11702 and R18 47R C25118 (0402 parts on 0603 lands) to C21190 and C23182; C7 R24..R35 180R C25270 (an 0805 part on 0603 lands) to C22828; C7 FB1..FB4 600R ferrite C1017 (an 0805 part on 0603 lands) to C1002; P1 Q1,Q2 CSD17570Q5B C2825431 (a switch) to C529279; B16 U1 C2830106 to C638299, U5 C2927919 to C93245, and the B16 list above (the M.2 socket codes are footprint-class stand-ins, not the named TE and Amphenol parts: the design session must confirm or pick the named parts). The package check was run over the laptop session's `design_codes.json` (every design code against the JLC package): no other mismatch. E6 U5 LT8705AIFE C674167 stays (correct part, 1 in stock). The corrected codes reach the BOMs at the next regeneration of each board (A23, B17, C8, D9, P2 per 32.66); until then the upload copies in this folder carry them.

**Stock shortfalls to carry to the order:** A22 C1,C2 100u 25V 1206 C2911627 (0 stock, pre-order); A22 C11 group 22u 50V X7R 1210 C55533070 (25 designators, pre-order only); A22 L1 XAL1010-103MED C6358489 (1 left since 12:11); B16 U13,U14 E72-2G4M20S1E C5352930 (0 stock); B16 U11 LG290P (78 in stock); D8 C58,C60 22p 500V 1206 (14 against 15); E6 U5 LT8705A (1); P1 F1 Keystone 3568 (2 against 5, bench-fitted). ILM placeholders (10k C25804 on A22 R90, R94, R98 and B16 R36: the BOM gives a current limit, not a resistor value) are owed by the design session. The stand-in inductors of the laptop session (A22 L2, L7, L8, L9, L10, L11; B16 L1, L101; C7 L1; E6 L2, L3) price only.
...
**Addendum 8 Sep 2026 12:45 CEST, the A22 line rebuilt so all 13 lines select (owner instruction 12:33):** Y25's Select Parts page keeps its auto-saved state (L1 to C2802562 XAL1010-153MED stand-in, U26 to C465737, U27,U28 to C2864778; 127 of 127 confirmed). The 169.26 of the earlier 304.50 PCB price was the "2-3 days" PCB build-time surcharge the re-opened line had picked up; "Change PCB specifications" showed the line's own options (6 layers, 240 x 170 with rails, 1.6 mm, black, ENIG, 1 oz / 0.5 oz, 0.25 mm via, epoxy filled and capped vias as the site set them, 8-9 days at 0.00) except the material, which sat on TG135; TG155 (3.68) and the JLC06161H-3313 stackup (the 1.5384 mm reminder, Yes) were set again and the PCB read 135.25. The specifications page loaded logged out once (Sign In, USD) and was fine on reload. SAVE TO CART returned "Network Error" but created **Y32**: PCB Y32-11651261A 135.25, PCBA SMT026090861766 1131.74 (one cent under Y25's 1131.75). Y25 (both lines) deleted from the cart. Cart at 12:44 with every line ticked: **13 items, merchandise 4594.90 EUR, shipping estimated 129.22, subtotal 4724.11**, est. shipping date 2026-09-17, 12.17 kg. `cart-830-set-2026-09-08.jpg` replaced with this screenshot (the earlier 11-item one is superseded). The A22 line now prices the corrected PCA9555 and 74LVC08 codes and the L1 stand-in; the C1,C2 and C11-group shortfalls stay as before.

## 5.2 EasyEDA Pro imports of the set, 8 Sep 2026 14:20 CEST

Every current V2 board is imported as a Private EasyEDA Pro project on the shared JLCPCB login (A22, B16 quote-only, C7, D8, E6, E5, P2; project ids and per-board notes in `v2/ecad/easyeda-pro/<board>-<rev>/import-notes.txt`, the archives under `v2/ecad/easyeda-pro/import/`, the method in `v2/ecad/easyeda-pro/README.md`). KiCad stays the source, the imports are one-way and unedited, nothing is on OSHWLab and no order is placed. The cart of section 5.1 stands as the reference for the day: 13 lines ticked, merchandise 4594.90 EUR, estimated shipping 129.22 EUR, subtotal 4724.11 EUR, screenshot `cart-830-set-2026-09-08.jpg`.
