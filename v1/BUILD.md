...
| power inlet (was USB-C) | 16.5 mm for the TY16 socket (the original hole was never measured: 12 to 16 mm) | front wall, reachable with the lid closed; the wall here is 3 mm or less. The DC-099 5.5 x 2.1 threaded jack of MESHSAT-793 (12 mm thread) sits in this hole until the IP68 TY16 back-nut socket replaces it. Open the hole to 16.5 mm with the step bit; the maker's drawing adds a flat at 15.3 mm, but a round hole works if the socket is held while the nut is tightened. The back-nut socket is rated for walls up to 3 mm; a thicker wall needs the two-hole flange version (16.5 mm plus two 3.5 mm holes 26 mm apart, M3 screws). |
...
### APRS chain, current state (17 September 2026)

**The APRS radio on both V1 kits is a PicoAPRS V4 VHF, not the UV-K5 and AIOC.** It replaced them
on 10 September 2026: a complete transceiver with its own KISS TNC over USB-C, so there is no sound
card, no Direwolf and no audio path. It sits on hub port 2 and enumerates as a CP2102N. **Every row
above and below that names the UV-K5 or the AIOC describes the first build and is superseded**,
including bulkhead 5, the component table, the plate contents and the commissioning steps. The
antenna actually fitted on the APRS bulkhead is not recorded here with confidence and should be
confirmed at the next bench session before this table is rewritten.

| | tesseract | parallax |
|---|---|---|
| Device ID | 255202693282056 | 180444492527880 |
| SW / Modem version | 26 / 22B | 26 / 22B |
| CP2102N serial | 36414a145c57ed11aeab52ca5720eef3 | 1cb04e3ba957ed11928b4aca5720eef3 |

The CP2102N serial is how to tell the two units apart without opening a case: it is the
`/dev/serial/by-id/` name the Bridge opens. Radio settings live on the units' buttons only and
survive no firmware update or factory reset, so re-read them after either: VHF 1 W, APRS 1 W, VHF
BW Wide on, FM Noise Cancel off, AFSK BW Wide on, AFSK RX volume about three quarters. Both units
were set the same on 17 September 2026.

**30 dB attenuator pads were tried on both APRS sockets on 16 September 2026 and removed the same
evening.** Each kit went deaf as its own pad was fitted, in both directions, and the link came back
when they came off. Do not re-order them.

**Rule: after anything touches an APRS connector, count frames before trusting the link.** Fitting
and removing the pads broke and remade every mated pair in both chains, and the link was left at 15
and 44 percent of frames decoded against a previous 65 to 83 percent. Re-seating both chains
restored 83 and 74 percent. The count is `rx` and `tx` on the Bridge's APRS status on both kits
seven minutes apart: the rate is the far kit's `rx` delta over this kit's `tx` delta. Frames lost
this way never arrive at all, so the corrupt-frame counters stay at zero and cannot warn you.
...
The X1202 takes four cells and charges from its USB-C or its barrel input. **Since MESHSAT-793 the standard is 12 V into the DC barrel (6 to 18 V, 3 A or more), from a Mean Well GST60A12-P1J brick, never USB-C at the same time; the case inlet is a DC-099 5.5 x 2.1 threaded jack with a rubber cap.** That cap seals only when nothing is plugged in, so **the inlet is not rain-proof while the kit is powered**. It is being replaced by a TY16 panel socket with screw terminals, which its maker rates IP68 mated or capped (a vendor rating of the connector, quoted; no rating is claimed for this kit, whose inlet has not been tested to anything) (ordered 18 September 2026, one set per kit: a 2-pin set on one kit and a 3-pin set with two pins used on the other, so the two kits' brick leads do not fit each other; label them). Inside, the bare ends of the 5.5 x 2.1 male pigtail go straight into the socket's terminals and the pigtail plugs into the X1202 as before. Outside, the barrel plug is cut off the brick cord and its two wires go into the cable plug, which seals on a round cable of 4.5 to 7.5 mm; find + with the multimeter first. On the 3-pin set the pins are on the brick lead, so plug the lead into the kit before the brick goes into the mains. Sealed means mated or capped, and the brick and its cord stay indoor-grade whatever the connector is. For a kit that stands outside for weeks (both kits go into the garden with sky view from late September 2026) the brick stays indoors and the 12 V lead is extended, never the mains side: the DC side is SELV, and a desktop brick in a home-made sealed box outdoors is neither rated nor dry. With 27 W at the input and 9 V kept at the kit, 2 x 0.5 mm2 reaches about 13 m, 2 x 1.0 mm2 about 27 m, 2 x 1.5 mm2 about 40 m; a 5 m run loses under 1 V on any of them. Use rubber H05RN-F or H07RN-F 2 x 1.0 mm2 for a permanent outdoor run (PVC H03VV-F works electrically but its sheath is not rated for sun, rain or frost), and check the outer diameter against the TY16 plug's 4.5 to 7.5 mm seal. The older text, kept because it describes the first build: the case USB-C inlet feeds it. The Pi is powered from the X1202 through the pogo pins. The Sabrent hub runs bus-powered from one Pi USB-A port: with the EEPROM setting of section 7 the Pi's USB budget is 1.6 A and the hub's four devices advertise about 900 mA together (AIOC 100, GPS 100, RTL-SDR 500, ZigBee 100). Keep the hub's barrel-jack aux cable in the spares pouch; it goes back in only if the assembled kit shows USB drops or modem resets in the field. The T-Call and the WiFi dongle sit on the Pi's other USB controller, not behind the hub.
